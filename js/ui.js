...
const UI = {
  ...
  updateNowPlaying(song) {
    ...
    if (artEl) {
      if (song.artwork) {
        artEl.innerHTML = '<img src="' + song.artwork + '" alt="' + this.escHtml(song.title) + '" onerror="this.onerror=null;this.parentElement.innerHTML=\'<<div class=\\\'art-placeholder\\\'>🎵</div>\'" loading="eager">';
      } else {
    ...
    if (fsArt && song.artwork) {
      fsArt.innerHTML = '<img src="' + song.artwork + '" alt="' + this.escHtml(song.title) + '" onerror="this.onerror=null;this.parentElement.innerHTML=\'<<div style=width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:80px;background:var(--bg-elevated)>🎵</div>\'">';
    }
  ...
  updatePlayerBgColor(song) {
    ...
    var colors = {
      "Pop": "#e74c3c22", "Hip-Hop": "#f39c1222", "R&B": "#9b59b622",
      "Rock": "#2c3e5022", "Electronic": "#3498db22", "K-Pop": "#e91e6322",
      "Jazz": "#79554822", "Classical": "#607d8b22", "Indie": "#27ae6022",
    };
  ...
  renderCard(song, index, songs) {
    ...
    var key = this._registerSong(song);
    var liked = State.liked.has(song.id);

    return '<div class="card stagger-item" onclick="Player.playSong(window.__songRegistry[\'s_' + song.id + '\'])" oncontextmenu="ContextMenu.open(event, ' + index + ')" data-id="' + song.id + '">' +
      '<div class="card-thumb">' +
        (song.artwork
          ? '<img src="' + song.artwork + '" alt="' + this.escHtml(song.title) + '" onerror="this.onerror=null;this.parentElement.innerHTML=\'<<div style=width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:40px;background:var(--bg-elevated)>🎵</div>\'" loading="lazy">'
  ...
  renderSongRow(song, index) {
    ...
    return '<div class="song-row ' + (active ? "active" : "") + '" onclick="Player.playSong(window.__songRegistry[\'s_' + song.id + '\'])" oncontextmenu="ContextMenu.open(event, ' + index + ')" data-id="' + song.id + '">' +
      '<div class="song-num">' +
      ...
      '<div class="song-heart ' + (liked ? "liked" : "") + '" onclick="event.stopPropagation(); State.toggleLike(\'' + song.id + '\'); this.classList.toggle(\'liked\', State.liked.has(\'' + song.id + '\')); this.innerHTML = State.liked.has(\'' + song.id + '\') ? \'<i class=\\\'fas fa-heart\\\'></i>\' : \'<i class=\\\'far fa-heart\\\'></i>\'">' +
  ...
  renderQueue() {
    ...
    if (State.queue.length === 0) {
      scroll.innerHTML = '<div style="text-align:center;padding:40px 16px;color:var(--text-muted);"><i class="fas fa-list" style="font-size:32px;margin-bottom:12px;display:block;"></i>Queue is empty</div>';
      return;
    }
    ...
    if (current) {
      ...
      html += '<div class="queue-item now">' +
        '<div class="q-thumb">' +
          (current.artwork
            ? '<img src="' + current.artwork + '" loading="lazy" onerror="this.onerror=null;this.parentElement.innerHTML=\'<<div style=width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:18px;background:var(--bg-elevated)>🎵</div>\'">'
    ...
    if (upcoming.length > 0) {
      ...
      upcoming.forEach(function(s, i) {
        ...
        html += '<div class="queue-item" onclick="Player.playSong(State.queue[' + realIdx + '])">' +
          '<div class="q-thumb">' +
            (s.artwork
              ? '<img src="' + s.artwork + '" loading="lazy" onerror="this.onerror=null;this.parentElement.innerHTML=\'<<div style=width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:18px;background:var(--bg-elevated)>🎵</div>\'">'
              : '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:18px;background:var(--bg-elevated)">🎵</div>') +
          '</div>' +
          '<div class="q-info">' +
            '<div class="q-title">' + UI.escHtml(s.title) + '</div>' +
            '<div class="q-artist">' + UI.escHtml(s.artist) + '</div>' +
          '</div>' +
          '<div class="q-dur">' + UI.formatTime(s.duration) + '</div>' +
        '</div>';
      });
    }
  ...
  renderLibrary() {
    ...
    var html = '<div class="section-header">' +
      '<div>' +
        ...
        '<div class="section-sub">' + liked.length + ' liked songs · ' + playlists.length + ' playlists</div>' +
      '</div>' +
    ...
    if (playlists.length > 0) {
      html += '<div class="section-header" style="margin-top:24px;"><div><div class="section-title" style="font-size:18px;">📁 Your Playlists</div></div></div>';
      html += '<div class="cards-grid">';
      playlists.forEach(function(pl) {
        html += '<div class="card" onclick="PlaylistManager.openPlaylist(\'' + pl.id + '\')" style="cursor:pointer;">' +
    ...
    if (liked.length === 0) {
      html += '<div class="no-results" style="margin-top:20px;"><i class="fas fa-heart"></i><h3>No liked songs yet</h3><p>Like songs to see them here</p></div>';
    } else {
      html += '<div class="section-header" style="margin-top:24px;"><div><div class="section-title" style="font-size:18px;">❤️ Liked Songs</div></div></div>';

      var likedSongs = liked.map(function(id) {
      ...
      if (likedSongs.length < liked.length) {
        ...
        var fetchPromises = missingIds.map(function(id) {
            ...
            .then(function(json) {
              if (json.results && json.results[0]) {
                ...
                window.__songRegistry["s_" + id] = {
                  id: String(t.trackId), title: t.trackName || "Unknown",
                  artist: t.artistName || "Unknown", album: t.collectionName || "Unknown",
                  duration: Math.round((t.trackTimeMillis || 0) / 1000),
                  previewUrl: t.previewUrl || null, artwork: art,
                  genre: t.primaryGenreName || "Music", source: "itunes",
                };
              }
            }).catch(function() {});
        });
  ...
  renderSidebarLibrary(songs) {
    ...
    if (allSongs.length === 0) {
      container.innerHTML = '<div style="padding:20px;text-align:center;color:var(--text-muted);font-size:13px;"><i class="fas fa-music" style="font-size:24px;margin-bottom:8px;display:block;"></i>Play some music!</div>';
      return;
    }

    container.innerHTML = '<div class="library-header">' +
    ...
    '</div>' +
    allSongs.slice(0, 15).map(function(s) {
      var key = UI._registerSong(s);
      return '<div class="library-item" onclick="Player.playSong(window.__songRegistry[\'s_' + s.id + '\'])">' +
        '<div class="library-thumb">' +
          (s.artwork
            ? '<img src="' + s.artwork + '" alt="" loading="lazy" onerror="this.onerror=null;this.parentElement.innerHTML=\'<<div style=width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:18px;background:var(--bg-elevated)>🎵</div>\'">'
            : '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:18px;background:var(--bg-elevated)">🎵</div>') +
        '</div>' +
